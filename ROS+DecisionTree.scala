...
val reducedROS = runROS.apply(reduced, 1.0, 0.0, 100)
val trainROSins = reducedROS.count()
val reduceTestdROS = runROS.apply(reducedTest, 1.0, 0.0, 100)
val testROSins = reduceTestdROS.count()
...
val model = DecisionTree.trainClassifier(reducedROS, numClasses, categoricalFeaturesInfo, impurity, maxDepth, maxBins)
val predictions = reduceTestdROS.map{ point=>
