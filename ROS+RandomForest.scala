...
val reducedROS = runROS.apply(reduced, 1.0, 0.0, 115)
val trainROSins = reducedROS.count()
val reducedTestROS = runROS.apply(reducedTest, 1.0, 0.0, 115)
val testROSins = reducedTestROS.count()
...
val model = RandomForest.trainClassifier(reducedROS, numClasses, categoricalFeaturesInfo, numTrees, featureSubsetStrategy, 
	impurity, maxDepth, maxBins) 
val predsAndLabels = reducedTestROS.map { point =>
